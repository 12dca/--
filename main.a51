...
TIME_H_1 DATA 3AH       
TIME_L_1 DATA 39H       
TIME_ADJUST_FLAG DATA 38H    //闪烁标志   
TIME_DELAY_FLAG DATA 37H
YEARH DATA 36H ;年份高两位
YEARL DATA 35H ;年份低两位存放单元(BCD码)
...
;	 SETB TR0    ;启动定时器
	 ...
	 MOV     R7, #84H
	 MOV     A, HOUR       ; 重新加载小时值
     ANL     A, #7FH       ; 清零最高位（第8位），保留低7位
     MOV     R6, A         ; 保存处理后的值到R6
     LCALL   DS1302_WRITE
	 LCALL   DS1302_Disable_Write         ; 开启写保护
	 MAIN:
